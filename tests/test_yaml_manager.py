...
from azure_functions_devops_build.constants import PYTHON, NODE, DOTNET, LINUX_CONSUMPTION, LINUX_DEDICATED, WINDOWS
...
class TestYamlManager(unittest.TestCase):
    ...
    def setUp(self):
        self._filename = "temp_{id}.yaml".format(id=id_generator())
    ...
    def test_create_yaml_python_linux(self):
        ...
        yaml_manager.create_yaml(filename=self._filename)
        yaml_path = os.path.join(self._current_directory, "pipeline_scripts", "python_linux.yml")
        result = TestYamlManager.is_two_files_equal(self._filename, yaml_path)
    ...
    def restore_file(filepath):
        if os.path.isfile("{}.bak".format(filepath)):
            os.rename("{}.bak".format(filepath), filepath)
